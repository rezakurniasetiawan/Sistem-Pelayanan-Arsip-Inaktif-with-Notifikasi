...
if ($foto != "") {
    ...
    if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
        ...
        if (!$result) {
            ...
            echo "<script>alert('Data berhasil ditambah.');window.location='../userPage.php';</script>";
        }
    } else {
    ...
    $query = "INSERT INTO tb_user (nama, email,nid, password, hakakses, bidang,created_at,created_up, foto) VALUES ('$nama','$email','$nid','$password','$hakakses','$bidang','$created_at','$created_up','450-person.png')";
    $result = mysqli_query($koneksi, $query);
    // periska query apakah ada error
    if (!$result) {
        die("Query gagal dijalankan: " . mysqli_errno($koneksi) .
